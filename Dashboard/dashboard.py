...
# Make sure the conversion worked and no NaT values exist
if hour_data['dteday'].isnull().any():
    st.write("There are some invalid date entries in 'dteday' column.")
else:
    ...
    if options == "Analisis Suhu & Cuaca":
        hour_data['hour'] = hour_data['dteday'].dt.hour  # Now we can safely access `.dt`
    ...
    if options == "Analisis Per Hari & Minggu":
        ...
        # Menambahkan visualisasi pola penyewaan sepeda berdasarkan hari dalam minggu
        # Mengelompokkan data berdasarkan hari dalam minggu dan menghitung rata-rata penyewaan
        hour_data['weekday'] = hour_data['dteday'].dt.weekday  # Access weekday after datetime conversion
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.barplot(x='day_name', y='cnt', data=weekday_data, palette='coolwarm')
        ax.set_title('Pola Sewa Sepeda Berdasarkan Hari dalam Seminggu')
